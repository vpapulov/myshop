...
from flask import Blueprint, send_from_directory, request, abort, render_template, flash, redirect, url_for
...
@images_blueprint.route('/product/<int:product_id>/new', methods=['GET', 'POST'])
@login_required
def image_new(product_id):
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            img_name = f'{product_id}-{hash(file)}-{secure_filename(file.filename)}'
    ...
    return render_template('image_new.html', product=product, title=f'Добавить файл для {product}')


@images_blueprint.route('/product/<int:product_id>/primary/resized/<int:size>', methods=['GET'])
@login_required
...
@images_blueprint.route('/product/<int:product_id>/<int:image_id>/resized/<int:size>', methods=['GET'])
@login_required
def image_resized(product_id, image_id, size):
    img = ProductImage.query.filter(
        and_(ProductImage.id == image_id, ProductImage.product_id == product_id)).first_or_404()
